      ...
      <div className="flex flex-row">
        <div className="flex">
          <input
            ...
            onBlur={()=>setShowSuggestionsDiv(false)}
          />
        ...
        {showSuggestionsDiv && <div className="flex flex-col bg-white shadow-lg absolute md:w-[30rem] sm:w-[10rem] mt-10 mx-1">
          <ul>
            {suggestionsArray.map((each_suggestion, index) => {
              return (
                <li className="rounded-lg border border-gray-200 px-3 py-2 cursor-pointer hover:bg-gray-100" key={index}>
                  {" 🔎 " + each_suggestion}
                </li>
              );
            })}
